...
/*   Updated: 2019/11/12 18:40:37 by tlandema         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int8_t	draw_infos_text(t_window *win, t_vm *env, int space_counter)
{
	...
	if ((tmp = draw_text(win, "- Cycles : ", point, text))
		...
		return (FAILURE);
	point.x += tmp;
	if ((str_tmp = ft_lltoa(env->current_cycles)) == NULL)
		return (FAILURE);
...
/*
** ici if ret == the_end on renvoie failure en r2alit2 c'est simplement la fin
...
int8_t			draw_infos(t_window *win, t_vm *env, int space_counter)
{
	...
	if (draw_rectangle(win, pos, create_color(50, 50, 44, 255)))
		return (FAILURE);
	if (draw_infos_text(win, env, space_counter) == FAILURE)
		return (FAILURE);
	if ((ret = check_infos_text(win, env, space_counter)) && ret == THE_END)
		return (FAILURE);
